...
\input{results/tables/hybrid_economics_macros.tex}
\input{results/tables/sce1_static_exit_analysis_macros.tex}
\input{results/tables/sce2_ro_results_fixed_macros.tex}  % Make sure Python exports \HostingROValue
\input{results/tables/sce3_ro_results_grid_macros.tex}   % Make sure Python exports \GridROValue

% \input{results/tables/basic_economics_macros.tex}
% \input{results/tables/hybrid_economics_macros.tex}
% \input{results/tables/static_exit_analysis_macros.tex}
% \input{results/tables/ro_results_fixed_macros.tex}
% \input{results/tables/simulation_setup_macros.tex}
% \input{results/tables/revenue_calibration_macros.tex}
...
\section{Numerical Implementation and Study Cases }
...
\begin{figure}[H]
    ...
    \caption{Historical dynamics of Bitcoin Price (left axis) and Hashprice (right axis). The tight coupling of these metrics serves as the calibration basis for the covariance matrix.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{results/figures/sce2_ro_value_fixed_cost.pdf}
...
When evaluated purely intrinsically—forcing immediate exercise without the flexibility to wait or switch—the traditional NPV under simulated paths is estimated at \textbf{\$\FixedPowerNPV}. However, when properly valuing the operational flexibility to weather market downturns and capitalize on bull runs, the Real Option Value is calculated at \textbf{\$\FixedPowerROValue}. 

The difference between these two metrics yields a flexibility premium of \textbf{\$\FixedPowerPremium}. This significant premium confirms that traditional NPV severely underprices mining assets by ignoring management's ability to truncate downside risk through operational suspension.

\subsection{Scenario 3: The "Grid" Case (Dual Stochasticity)}
In the final scenario, the firm bypasses the fixed-rate hosting provider and connects directly to the wholesale electricity grid (ERCOT). This exposes the operation to significant energy price volatility ($Y_t$). However, it also grants the firm the \textit{Spark Spread Option}—the ability to dynamically curtail operations during acute power price spikes while benefiting from significantly lower average off-peak rates.

To value this, the Hedged Monte Carlo engine calibrates a replicating portfolio using historical ERCOT spot prices (as a proxy for short-term power futures). The algorithm determines the optimal two-dimensional stopping boundary across both Bitcoin and Energy prices.

Embracing energy volatility significantly alters the strategic value of the project. In this dual-stochastic environment, the intrinsic NPV is estimated at \textbf{\$\GridNPV}, but the Real Option value rises dramatically to \textbf{\$\GridROValue}. This generates a massive flexibility premium of \textbf{\$\GridPremium}.

\paragraph{Decomposition of Option Value:}
Comparing the three scenarios reveals the true sources of strategic value in cryptocurrency mining. Traditional DCF and static NPV severely misprice these assets because they fail to account for management's ability to asymmetrically capture upside volatility while truncating downside risk. By securing the operational flexibility to suspend operations during adverse price movements—in both the crypto and energy markets—miners can extract significant value from the inherent volatility of the asset class.
